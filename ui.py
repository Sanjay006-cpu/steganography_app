...
from PySide6.QtWidgets import (
    ...
    QPushButton, QSlider, QCheckBox, QGroupBox, QRadioButton, QTextEdit,
...
class SteganographyApp(QWidget):
    ...
    def init_ui(self):
        ...
        self.key_display.setPlaceholderText("Key will appear here after encoding")
        self.key_layout.addWidget(self.key_label)
        self.key_layout.addWidget(self.key_display)
    ...
    def update_action(self):
        ...
        self.key_widget.setVisible(self.action == "encode")
        self.reset_files()
    ...
    def select_output_file(self):
        ...
        default_file_name = (
            os.path.splitext(self.input_file)[0] + "_stego" + os.path.splitext(self.input_file)[1]
            if self.input_file else ""
        )
        file_name, _ = QFileDialog.getSaveFileName(self, "Select Output File", default_file_name, file_types)
        if file_name:
            self.output_file = file_name
            self.output_file_label.setText(os.path.basename(file_name))

    def update_char_count(self):
        current_text = self.data_entry.toPlainText()
        ...
        if self.action == "encode" and self.max_chars > 0:
            if current_length > self.max_chars:
                self.data_entry.setPlainText(current_text[:self.max_chars])
                cursor = self.data_entry.textCursor()
                cursor.movePosition(QTextCursor.End)
                self.data_entry.setTextCursor(cursor)
                current_length = self.max_chars
                current_bytes = len(self.data_entry.toPlainText().encode('utf-8'))
            self.char_count_label.setText(f"Characters: {current_length} / {self.max_chars}, Bytes: {current_bytes}")
        else:
            self.char_count_label.setText(f"Characters: {current_length}, Bytes: {current_bytes}")
            ...
            if self.action == "encode":
                ...
                if self.auto_key_radio.isChecked():
                    ...
                    QMessageBox.information(
                        self,
                        "Success",
                        "Encoding complete. (copied to clipboard 📋)"
                    )
