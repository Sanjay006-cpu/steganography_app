...
from password import RateLimiter
...
class SteganographyApp(QWidget):
    ...
    def init_ui(self):
        ...
        # Style sheet
        self.setStyleSheet("""
            QWidget { background-color: black; color: white; font-family: 'Segoe UI'; }
            QGroupBox { font-weight: bold; border: 1px solid white; border-radius: 5px; padding: 10px; }
            QPushButton { background-color: #333333; color: white; border-radius: 6px; padding: 6px; }
            QPushButton:hover { background-color: #555555; }
            QRadioButton, QCheckBox { color: white; }
            QTextEdit, QLineEdit { background-color: #222222; color: white; border: 1px solid white; }
        """)
        ...
        # TOTP QR Code Button
        self.qr_button = QPushButton("Show TOTP QR Code")
        self.qr_button.clicked.connect(self.show_totp_qr)
        main_layout.addWidget(self.qr_button, alignment=Qt.AlignCenter)
    ...
    def select_input_file(self):
        ...
        if file_name:
            ...
            max_bytes = handler.get_max_data_size(file_name) - 16
        ...
        try:
            if self.action == "encode":
                ...
                if self.auto_key_radio.isChecked():
                    ...
                    QMessageBox.information(self, "Success", "Encoding complete. Key copied to clipboard.")
                else:
                    ...
                    handler.encode(self.input_file, hidden_data, self.output_file)
                    self.progress_bar.setValue(100)
                    QMessageBox.information(self, "Success", "Encoding complete. Remember your password.")
                    self.password = None
    ...
    def show_totp_qr(self):
        try:
            uri = self.totp.provisioning_uri(name="SteganographyApp", issuer_name="YourApp")
            qr = qrcode.make(uri)
            ...
            qr_dialog.setText("Scan this QR code with your Microsoft Authenticator app:")
            qr_dialog.setIconPixmap(pixmap)
            qr_dialog.exec()
        except Exception as e:
            logging.error(f"Failed to generate or display QR code: {e}")
